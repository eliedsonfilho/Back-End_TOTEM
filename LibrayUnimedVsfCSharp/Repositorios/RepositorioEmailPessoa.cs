...
{
    public class RepositorioEmailPessoa : Repositorio, IRepositorio<EmailPessoa, int>
    {
        public EmailPessoa ObterPorId(int autoIdBoleto, bool lazy)
        {
            IDbCommand command;
            //IDataReader dataReaderTmp;
            EmailPessoa objetoPesquisado = new EmailPessoa();
            ...
            try
            {
                command = new SqlCommand(@"Select 
                                            *
                                            From 
                                            EmailPessoa where autoId = @autoId");
        ...
        {
            //IDataReader dataReaderTmp;
            IList<EmailPessoa> listaObjetosPesquisados = null;
            EmailPessoa objetoPesquisado = new EmailPessoa();
        ...
        public EmailPessoa ObterPorPessoa(Pessoa pessoa, bool lazy)
        {
            ...
            try
            {
                command = new SqlCommand(@"Select top 1
                                            *
                                            From 
                                            EmailPessoa where Pessoa = @Pessoa 
                                            and FimVigencia is null
                                            ORDER BY Seq Desc");

                SqlParameter parameterautoId = new SqlParameter("@Pessoa", pessoa.AutoId);
                command.Parameters.Add(parameterautoId);
